...
# Add repo and intall php 7.2 and nginx
...
&& sed -i "s#memory_limit = 128M#memory_limit = 512M#" /etc/php/7.2/fpm/php.ini \
&& sed -i "s#;always_populate_raw_post_data = -1#always_populate_raw_post_data = -1#" /etc/php/7.2/fpm/php.ini \
&& sed -i "s#post_max_size = 8M#post_max_size = 100M#" /etc/php/7.2/fpm/php.ini \
&& sed -i "s#max_execution_time = 60#max_execution_time = 120#" /etc/php/7.2/fpm/php.ini \
# CLI
...
RUN cat /tmp/xdebug.ini >> /etc/php/7.2/fpm/php.ini \
&& export XDEBUG_SOURCE=`find /usr/lib -name "xdebug.so"` \
&& sed -i "s#xdebug_so_path#$XDEBUG_SOURCE#" /etc/php/7.2/fpm/php.ini

# Xdebug setting for the command line
RUN cat /tmp/xdebug.ini >> /etc/php/7.2/cli/php.ini \
&& export XDEBUG_SOURCE=`find /usr/lib -name "xdebug.so"` \
&& sed -i "s#xdebug_so_path#$XDEBUG_SOURCE#" /etc/php/7.2/cli/php.ini
...
&& echo 'sendmail_path = /usr/bin/mhsendmail --smtp-addr mailhog:1025' >> /etc/php/7.2/fpm/php.ini
